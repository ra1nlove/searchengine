...
public class SimHash {
    ...
    static int hashbits = 64;

    public static String simHash(String text) throws IOException {
        ...
        for(String word : wordList) {
            // 1、将每一个分词hash为一组固定长度的数列.比如 64bit 的一个整数.
            BigInteger t = hash(word);
            for (int i = 0; i < hashbits; i++) {
                BigInteger bitmask = new BigInteger("1").shiftLeft(i);
                // 2、建立一个长度为64的整数数组(假设要生成64位的数字指纹,也可以是其它数字),
    ...
    public static void main(String[] args) throws IOException {
        ...
        String hash1 = SimHash.simHash(s);
        ...
        String hash2 = SimHash.simHash(s);
        ...
        String hash3 = SimHash.simHash(s);
    }
}